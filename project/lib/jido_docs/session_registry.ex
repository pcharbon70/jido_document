defmodule JidoDocs.SessionRegistry do
  ...
  alias JidoDocs.{Error, SignalBus}
  ...
  def init(opts) do
    state = %{
      ...
      path_index: %{},
      monitors: %{},
      session_supervisor: Keyword.get(opts, :session_supervisor, JidoDocs.SessionSupervisor),
  ...
  defp start_new_session(state, session_id, path, opts) do
    ...
    case DynamicSupervisor.start_child(state.session_supervisor, {JidoDocs.Agent, agent_opts}) do
      {:ok, pid} ->
