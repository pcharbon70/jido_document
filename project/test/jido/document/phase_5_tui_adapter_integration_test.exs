defmodule Jido.Document.Phase5TuiAdapterIntegrationTest do
  use ExUnit.Case, async: true

  alias Jido.Document.Adapters.TUI
  ...
  setup do
    ...
    tmp_dir = Path.join(File.cwd!(), "tmp_jido_document_phase5_tui_" <> uniq)
  ...
  defp await_signal(type) do
    receive do
      {:jido_document_signal, %Signal{type: ^type} = signal} ->
        signal

      {:jido_document_signal, _signal} ->
        await_signal(type)
    after
