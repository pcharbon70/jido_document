defmodule JidoDocs.Phase5AdapterCoordinationIntegrationTest do
  use ExUnit.Case, async: true

  alias JidoDocs.Adapters.{DesktopCoordinator, LiveView, TUI}
  ...
  setup do
    ...
    tmp_dir = Path.join(File.cwd!(), "tmp_jido_docs_phase5_coord_" <> uniq)
  ...
  test "liveview and tui remain consistent while sharing one session", ctx do
    ...
    assert {:noreply, live} = LiveView.handle_info(live, {:jido_docs_signal, updated})
    ...
    assert {:noreply, live} = LiveView.handle_info(live, {:jido_docs_signal, rendered})
  ...
  defp await_signal(type, session_id) do
    receive do
      {:jido_docs_signal, %Signal{type: ^type, session_id: ^session_id} = signal} ->
        signal

      {:jido_docs_signal, _other} ->
