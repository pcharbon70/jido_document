defmodule JidoDocs.Phase5LiveViewAdapterIntegrationTest do
  use ExUnit.Case, async: true

  alias JidoDocs.Adapters.LiveView
  alias JidoDocs.Field
  alias JidoDocs.{SessionRegistry, SessionSupervisor, SignalBus}
  ...
  setup do
    ...
    tmp_dir = Path.join(File.cwd!(), "tmp_jido_docs_phase5_liveview_" <> uniq)
  ...
  test "mounts by path, updates via events, and maps lock signals into assigns", ctx do
    ...
    assert_receive {:jido_docs_signal, %{data: %{action: :lock_state}} = signal}, 500
    assert {:noreply, adapter} = LiveView.handle_info(adapter, {:jido_docs_signal, signal})
