defmodule Jido.Document.Phase4SchedulingFallbackIntegrationTest do
  use ExUnit.Case, async: false

  alias Jido.Document.{Agent, Error, Signal}
  alias Jido.Document.Render.JobQueue
  ...
  test "queue saturation and superseded cancellation keep latest render jobs" do
    ...
    assert_receive {:jido_document_render_job, "session-a", 2, {:ok, _preview}, _meta}, 500
    assert_receive {:jido_document_render_job, "session-b", 1, {:ok, _preview}, _meta}, 500
    refute_receive {:jido_document_render_job, "session-a", 1, _result, _meta}, 50
  ...
  test "renderer failure triggers fallback preview then recovery signal on success" do
    ...
    tmp_dir = Path.join(System.tmp_dir!(), "jido_document_phase4_" <> session_id)
    ...
    assert_receive {:jido_document_signal, %Signal{type: :failed, session_id: ^session_id}}, 500
